...
import { Target, Plus, TrendingUp, Wallet, Star, CheckCircle2, ChevronRight } from 'lucide-react';
...
const GoalTracker: React.FC<GoalTrackerProps> = ({ goals, onAddGoal, onUpdateGoalAmount, currencySymbol }) => {
    ...
    <div className="space-y-6 pb-24 animate-fade">
      ...
      <div className="grid gap-4">
        ...
        {goals.map((goal) => {
          ...
          return (
            <div key={goal.id} className={`bg-slate-900/60 backdrop-blur-3xl p-6 rounded-[2.5rem] border border-white/5 space-y-4 transition-all hover:bg-slate-900/80 ${isCompleted ? 'border-emerald-500/30' : ''}`}>
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">الهدف: {goal.targetAmount.toLocaleString()} {currencySymbol}</p>
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-[9px] font-black text-slate-500 uppercase tracking-widest">
                  ...
                  <span>المتبقي: {(goal.targetAmount - goal.currentAmount).toLocaleString()}</span>
                </div>
                <div className="h-3 bg-slate-950 rounded-full overflow-hidden p-[2px]">
                   <div className={`h-full rounded-full transition-all duration-1000 ${isCompleted ? 'bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.5)]' : 'bg-amber-500'}`} style={{ width: `${progress}%` }} />
                </div>
              </div>
            </div>
          );
          ...
          <div className="bg-slate-900 w-full max-w-md rounded-t-[3.5rem] sm:rounded-[4rem] p-10 shadow-2xl border-t border-white/5 animate-slide-up">
            ...
            <div className="space-y-6">
               <div className="space-y-2">
                 ...
                 <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="مثلاً: رحلة الأحلام، سيارة جديدة" className="w-full p-5 rounded-2xl bg-slate-950 text-white font-bold border-none outline-none focus:ring-1 focus:ring-amber-500" />
               </div>
               <div className="space-y-2">
                 <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-2">المبلغ المستهدف</label>
                 <input type="number" value={target} onChange={e => setTarget(e.target.value)} placeholder="0.00" className="w-full p-5 rounded-2xl bg-slate-950 text-white font-black border-none outline-none focus:ring-1 focus:ring-amber-500 text-center text-3xl" />
               </div>
               <button onClick={() => {
                 if (name && target) {
                   onAddGoal({ name, targetAmount: parseFloat(target), currentAmount: 0, color: '#f59e0b', icon: 'Star' });
