...
import { Plus, LayoutDashboard, History, Settings as SettingsIcon, BrainCircuit, HandCoins, Repeat, Coins, ArrowRight } from 'lucide-react';
          ...
          {activeTab === 'dashboard' && (
            <div className="flex items-center gap-2">
                <div className="flex-1 overflow-hidden mask-gradient-x py-2 -my-2">
                    ...
                    {[...state.currencies, ...state.currencies, ...state.currencies].map((curr, index) => {
                        ...
                        <button
                            ...
                            className={`
                                relative flex items-center gap-3 pl-5 pr-3 py-3 rounded-full border backdrop-blur-md transition-all duration-500
                                ${isActive 
                                    ? 'bg-amber-500 border-amber-400 text-slate-950 shadow-[0_0_25px_rgba(245,158,11,0.5)] scale-105 z-10' 
                                    : 'bg-slate-800/30 border-white/5 text-slate-400 hover:bg-slate-800 hover:border-white/20 hover:scale-105'
                                }
                            `}
                        >
                            ...
                            <span className={`text-[10px] font-black uppercase tracking-widest ${isActive ? 'text-slate-900/60' : 'text-slate-500'}`}>
                                {curr.code}
                            </span>
                ...
                <button 
                    ...
                    className="flex items-center justify-center w-12 h-12 rounded-full bg-slate-800/50 border border-slate-700 text-slate-500 hover:text-amber-500 hover:border-amber-500/50 transition-all shrink-0 active:scale-95 backdrop-blur-md shadow-lg z-20"
                >
                    <Plus size={18} />
                </button>
            </div>
            ...
            {activeTab === 'dashboard' && (
              <>
                ...
                <section className="space-y-4">
                  ...
                  <TransactionList 
                    transactions={currentCurrencyTransactions.slice(0, 5)} 
                    categories={state.categories} 
                    wallets={state.wallets} 
...
const NavButton = ({ icon, label, active, onClick }: { icon: any, label: string, active: boolean, onClick: () => void }) => (
  <button onClick={onClick} className={`flex flex-col items-center gap-1 transition-all flex-1 group ${active ? 'text-amber-500' : 'text-slate-500'}`}>
    ...
    <span className={`text-[9px] font-bold transition-all duration-300 ${active ? 'opacity-100 translate-y-[-8px]' : 'opacity-0 scale-0 h-0'}`}>
        {label}
    </span>
  </button>
);
